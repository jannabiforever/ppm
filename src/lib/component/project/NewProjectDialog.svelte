...
<Dialog.Root bind:open={isDialogOpen}>
	<Dialog.Trigger
		class="card border-surface-300-700 hover:bg-surface-100-900 flex flex-col items-center justify-center border border-dashed p-6 transition-all duration-200"
		onclick={() => (isDialogOpen = true)}
	>
		<div class="bg-surface-100-900 mb-3 rounded-full p-3">
			<Plus />
		</div>
	...
	<Dialog.Portal>
		...
		>
			...
			<form class="grid gap-6" method="POST" action="?/addNewProject" use:enhance={handleEnhance}>
				<FormField
					id="name"
					name="name"
					label="Name"
