...
에러 정의 파일. `Data.TaggedError`를 활용해 에러를 정의. 자세한 사항은 error-handling.md를 참고
...
서비스 자체에는 주석을 달지 않고, 대신 인터페이스들에 주석을 달아 사용한다.
...
이때 각 인터페이스 메서드는 기본적으로 `types.ts`에 정의된 스키마 상수들을 활용하며,
받는 인수는 `typeof Schema.Encode`이고 반환하는 Effect의 성공 타입은 `typeof Schema.Type`이어야 한다.
...
스키마 상수 정의. (`effect/Schema` 활용)

아래 예시와 같이 구성. 모든 타입은 그에 대응되는 스키마를 포함함.
만약 해당 서비스와 관련된 rpc 함수나, 뷰가 있다면 그에 맞는 스키마가 모두 정의되어야 함.
...
`lib/shared/schema/index.ts`에 정의되어 있는 `Equal` 헬퍼 타입을 사용해서 구성함:
