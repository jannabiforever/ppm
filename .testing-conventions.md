...
├── __tests__/
│   ├── [service_name].test.ts       # Service integration tests
│   ├── [service_name].unit.test.ts  # Business logic unit tests
...
src/__tests__/
├── setup/
│   ├── test-environment.ts         # Effect runtime setup
│   ├── mock-supabase.ts            # Supabase mocking utilities
│   ├── test-helpers.ts             # Common test utilities
│   └── test-clock.ts               # Time testing utilities
└── fixtures/
    ├── global/                     # Shared across modules
    │   ├── users.ts
    │   ├── projects.ts
    │   └── common-errors.ts
    └── database/                   # Database-specific test data
        ├── seeds/                  # Initial data sets
        ├── migrations/             # Test migration helpers
...
### 1. Supabase Service Mocking
```typescript
// src/__tests__/fixtures/mock-services.ts
export const createMockSupabaseService = (config: {
  insertResult?: any;
  selectResult?: any;
  shouldFail?: boolean;
  error?: Error;
  ...
  return Layer.succeed(SupabaseService, {
    getClientSync: () => Effect.succeed({
      from: (table: string) => ({
        insert: () => ({
          select: () => ({
            single: () => Promise.resolve({
              data: config.shouldFail ? null : config.insertResult,
        ...
        select: () => ({
          eq: () => ({
            maybeSingle: () => Promise.resolve({
              data: config.shouldFail ? null : config.selectResult,
              error: config.shouldFail ? config.error : null
            })
          })
        })
...
```typescript
export const createMockTaskService = (overrides: Partial<TaskService> = {}) => {
...
describe('CreateTaskSchema', () => {
  test('should validate valid input', () => {
    ...
    const result = Schema.decodeUnknownSync(CreateTaskSchema)(validInput);
  ...
  test('should reject invalid input', () => {
    ...
    expect(() =>
      Schema.decodeUnknownSync(CreateTaskSchema)(invalidInput)
    ).toThrow();
  });
});
```
...
describe('TaskService Business Logic', () => {
  test('validateTaskStatusTransitionSync should allow valid transitions', async () => {
    const result = await Effect.runPromise(
      taskService.validateTaskStatusTransitionSync('planned', 'in_session')
    );
    expect(result).toBeUndefined(); // void success
  });

  test('validateTaskStatusTransitionSync should reject invalid transitions', async () => {
    const effect = taskService.validateTaskStatusTransitionSync('completed', 'in_session');
...
describe('FocusSessionService Integration', () => {
  test('complete session lifecycle', async () => {
    ...
    const closedSession = await Effect.runPromise(
      focusSessionService.endFocusSessionAsync(session.id, {
...
```typescript
// src/lib/modules/focus_session/__tests__/fixtures/mock-data.ts
...
### 2. Global Shared Fixtures
```typescript
// src/__tests__/fixtures/global/users.ts
...
```typescript
// src/__tests__/fixtures/database/seeds/focus-session-scenarios.ts
export const focusSessionScenarios = {
  activeSession: () => ({
    session: createMockSession({ closed_at: null }),
    tasks: [
      createMockTask({ id: 'task_1', status: 'in_session' }),
      createMockTask({ id: 'task_2', status: 'in_session' })
    ],
    sessionTasks: [
      createMockSessionTask({ task_id: 'task_1', order_index: 1 }),
      createMockSessionTask({ task_id: 'task_2', order_index: 2 })
    ]
  }),
  ...
  completedSession: () => ({
    ...
    tasks: [
      createMockTask({ id: 'task_1', status: 'completed' }),
      createMockTask({ id: 'task_2', status: 'planned' })
    ],
    sessionTasks: [
      createMockSessionTask({
        task_id: 'task_1',
...
### 4. Fixture Cleanup
```typescript
// src/__tests__/fixtures/database/cleanup/index.ts
export const cleanupFixtures = {
  async sessions() {
    await Promise.all([
      testSupabase.from('session_tasks').delete().neq('id', ''),
      testSupabase.from('focus_sessions').delete().neq('id', '')
    ]);
  },
...
test('should handle user creation', () => {
  const testUser = createMockUser({ id: 'test-user-1' });
  // Use testUser in this test only
});

// ❌ BAD: Shared mutable test data
const globalTestUser = { id: 'shared-user' };
test('should update user', () => {
  globalTestUser.name = 'Updated'; // Affects other tests
});
```
...
describe('Task Schemas', () => {
  describe('CreateTaskSchema', () => {
    test('should validate minimum required fields', () => {
      const input = { title: 'Test Task' };
      const result = Schema.decodeUnknownSync(CreateTaskSchema)(input);
      expect(result.title).toBe('Test Task');
    });

    test('should enforce title length limits', () => {
      const input = { title: 'a'.repeat(201) }; // Exceeds 200 char limit

      expect(() =>
        Schema.decodeUnknownSync(CreateTaskSchema)(input)
      ).toThrow();
    });
  });
  ...
  describe('TaskStatusSchema', () => {
    ...
    test('should reject invalid status values', () => {
      ...
      ).toThrow(/Expected one of \["backlog", "planned", "in_session", "blocked", "completed"\]/);
    });

    test('should validate schema structure on failure', () => {
      const invalidInput = {
        title: '', // Empty title
...
- **Consistent testing patterns** across all Effect.ts services
- **Proper isolation** of external dependencies
- **Comprehensive coverage** of business logic and error scenarios
- **Maintainable test code** that follows Effect.ts best practices
- **Fast, reliable test execution** with proper mocking strategies

Follow these conventions to build a robust test suite that provides confidence in your Effect.ts service architecture.
