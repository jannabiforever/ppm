...
test('should test service with dependencies', async () => {
	...
	}).pipe(Effect.provide(TestServiceLive), Effect.runPromise);
...
test('createTaskAsync should create task and return result', async () => {
	...
	const result = await taskService
		.createTaskAsync(validInput)
		.pipe(Effect.provide(mockSupabase), Effect.runPromise);
...
test('should handle database errors gracefully', async () => {
	...
	await expect(
		taskService.createTaskAsync(validInput).pipe(Effect.provide(mockSupabase), Effect.runPromise)
...
test('should handle business rule violations', async () => {
	...
	await expect(effect.pipe(Effect.provide(mockLayer), Effect.runPromise)).rejects.toThrow(
...
export const createMockFocusSessionService = (overrides = {}) => {
	const defaultMethods = {
		...
		endFocusSessionAsync: () =>
			Effect.succeed({ ...mockFocusSession, closed_at: new Date().toISOString() }),
...
const createTestLayers = (config: TestLayerConfig = {}) => {
	...
	return FocusSessionLive.pipe(Layer.provide(Layer.mergeAll(mockSupabaseLayer, mockTaskLayer)));
};
```
...
describe('FocusSessionService Integration', () => {
	test('should complete full session workflow', async () => {
		...
		// Start session
		const session = await focusSessionService
		...
		// End session
		const closedSession = await focusSessionService
			.endFocusSessionAsync(session.id, {
				task_completion_updates: [{ task_id: 'task1', completed: true, seconds_spent: 1200 }]
			})
			.pipe(Effect.provide(mockLayers), Effect.runPromise);
...
describe('TaskService Business Logic', () => {
	test('validateTaskStatusTransitionSync should allow valid transitions', async () => {
		const result = await taskService
			.validateTaskStatusTransitionSync('planned', 'in_session')
			.pipe(Effect.runPromise);
	...
	test('calculateSessionDurationSync should compute correct duration', async () => {
		...
		const duration = await focusSessionService
			.calculateSessionDurationSync(session)
			.pipe(Effect.runPromise);
	...
	test('should handle database connection failures', async () => {
		...
		await expect(
			focusSessionService
				...
				.pipe(Effect.provide(errorLayer), Effect.runPromise)
...
test('should prevent race conditions in task status updates', async () => {
	...
	const finalTask = await taskService.getTaskByIdAsync(taskId).pipe(Effect.runPromise);
...
describe('Layer isolation tests', () => {
	...
	beforeEach(() => {
		...
		baseTestLayers = Layer.mergeAll(createMockSupabaseService(), createMockTaskService());
	});
...
test('should rollback on transaction failure', async () => {
	const invalidInput = {
		/* data that will cause constraint violation */
	};
...
test('should propagate errors through Effect chains', async () => {
	...
	const activeSessions = await focusSessionService
...
test('should handle partial failures gracefully', async () => {
	...
	if ('error' in result) {
		...
		const validTasks = await taskService
			.getTasksAsync({
...
export const focusSessionScenarios = {
	...
	emergencyCloseSession: {
		...
		endInput: {
			task_completion_updates: [{ task_id: 'task_1', completed: false, seconds_spent: 2700 }]
		},
...
export const createMockFocusSessionService = (overrides = {}) => {
	return Layer.succeed(FocusSessionService, {
		startFocusSessionAsync: () => Effect.succeed(mockFocusSession),
		endFocusSessionAsync: () =>
			Effect.succeed({ ...mockFocusSession, closed_at: '2024-01-01T10:45:00Z' }),
...
const testLayers = FocusSessionLive.pipe(
	Layer.provide(
		Layer.mergeAll(createMockSupabaseService(config), createMockTaskService(taskOverrides))
	)
);
```
...
describe('TaskService', () => {
	...
	describe('createTaskAsync', () => {
		test('should create task successfully', async () => {
			...
			}).pipe(Effect.provide(Layer.mergeAll(TaskLive, mockSupabase)), Effect.runPromise);
		...
		test('should handle database errors', async () => {
			...
			await expect(
				Effect.gen(function* () {
					const taskService = yield* TaskService;
					return yield* taskService.createTaskAsync(validCreateTaskInput);
				}).pipe(Effect.provide(Layer.mergeAll(TaskLive, errorSupabase)), Effect.runPromise)
			).rejects.toMatchObject({
				_tag: 'SupabasePostgrestError',
...
describe('Task Schemas - Business Critical Only', () => {
	...
	describe('TaskStatusSchema', () => {
		...
		test('should reject invalid status that could break business logic', () => {
			expect(() => Schema.decodeUnknownSync(TaskStatusSchema)('invalid_status')).toThrow();
		});
	});
});
...
describe('TaskService Integration - Input Validation', () => {
	test('should handle invalid input gracefully in business workflow', async () => {
		...
		await expect(
			taskService.createTaskAsync(invalidTask).pipe(Effect.provide(mockLayers), Effect.runPromise)
		).rejects.toThrow();
	});
});
```
